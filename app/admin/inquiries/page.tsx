...
import { MessageSquare, Mail, Phone, Building2, Calendar, CheckCircle } from 'lucide-react';
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-6xl mx-auto flex flex-col gap-8 pb-12 h-[calc(100vh-140px)]">
      ...
      {loading ? (
        ...
        <div className="flex-1 flex flex-col lg:flex-row gap-6 min-h-0">
                ...
                inquiries.map((inq) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-1">
                      ...
                      <span className="text-[10px] font-bold text-slate-400 whitespace-nowrap">{new Date(inq.createdAt).toLocaleDateString()}</span>
                    </div>
          ...
          {/* Details View */}
          <div className="w-full lg:w-2/3 bg-white/80 backdrop-blur-xl rounded-3xl border border-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-y-auto p-6 lg:p-8">
            {selectedInquiry ? (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col h-full">
                <div className="flex justify-between items-start border-b border-slate-100 pb-6 mb-6">
                  ...
                  <button className="flex items-center gap-2 px-4 py-2 bg-emerald-50 text-emerald-600 font-bold rounded-xl hover:bg-emerald-100 transition-colors">
